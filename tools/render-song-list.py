...
It rewrites the genre jump links, the genre blocks and the song count on the
...
def main():
    ...
    jump = '<ul class="songs__jump">' + "".join(
        f'<li><a href="#genre-{slug(g["genre"])}">{html.escape(g["genre"])}</a></li>' for g in genres) + "</ul>"
    groups = []
    for g in genres:
        items = []
        for s in g["songs"]:
    ...
    page, n1 = re.subn(r'<ul class="songs__jump">.*?</ul>', jump, page, count=1, flags=re.S)
